...
@Config
@TeleOp(name = "PID Auto-Tuning_dumb", group = "TeleOp")
public class PIDautoTuning_dumbModel extends LinearOpMode {
    ...
    private boolean moveToTargetWithRotation(Vector2d targetPosition, double targetAngle) {
        double totalErrorMove = 0.0;
        double totalErrorTurn = 0.0;
        int steps = 0;
        ...
        while (opModeIsActive()) {
            ...
            // Суммируем ошибки для последующей настройки коэффициентов
            totalErrorMove += errorMagnitude;
            totalErrorTurn += Math.abs(angleError);
            steps++;
        ...
        // Настроим коэффициенты PID после завершения движения
        if (steps > 0) {
            double averageErrorMove = totalErrorMove / steps;
            double averageErrorTurn = totalErrorTurn / steps;

            tunePIDCoefficients(averageErrorMove, averageErrorTurn);
        }
